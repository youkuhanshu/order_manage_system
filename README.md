...
│   └── Comment_service.h        # CommentService 类（评论排序、好评推荐）
│
├── storage/
...
不在 `ui/src/` 里的后端源文件，需要在 `CMakeLists.txt` 里手动追加（已经加好的有四个）：
...
list(APPEND srcs ${CMAKE_CURRENT_SOURCE_DIR}/../comment/src/comment_service.cpp) # 评论排序
```
